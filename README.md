...
- **Система аутентификации сумок** с уникальными идентификаторами для безопасного подключения
...
Каждая доставочная сумка имеет уникальный идентификатор для безопасного подключения и предотвращения несанкционированного доступа.

#### **Идентификация сумки**

- **Формат ID:** `SB000001`, `SB000002`, `SB000003` и т.д.
- **Уникальность:** каждой сумке присваивается индивидуальный ID при прошивке
- **Хранение:** ID жестко задан в прошивке Arduino (`#define BAG_ID "SB000001"`)

#### **Процесс аутентификации**

1. **Отправка идентификатора:** сумка автоматически отправляет свой ID каждые 5 секунд до получения подтверждения
2. **Подтверждение:** приложение отвечает командой подтверждения для конкретного ID
3. **Установка соединения:** после успешного подтверждения начинается обмен основными данными
4. **Поддержание связи:** для предотвращения потери соединения ID переотправляется каждые 60 секунд

#### **Команды аутентификации**

**От сумки к приложению:**

```
ID:SB000001
```

**От приложения к сумке:**

```
ID_OK:SB000001    # Подтверждение правильного ID
```

#### **Поведение при переподключении**

- **Автоматический сброс:** через 60 секунд после последнего подтверждения система возвращается в режим поиска
- **Повторная аутентификация:** при каждом переподключении процесс аутентификации повторяется
- **Защита от ошибок:** неправильные ID игнорируются, подключение возможно только с верным идентификатором
...
- **Система аутентификации:** каждая сумка идентифицируется уникальным ID
- **Предотвращение несанкционированного доступа:** подключение возможно только к сумкам с известными идентификаторами
...
4. **Дождитесь завершения аутентификации** (приложение автоматически распознает ID сумки)
5. Используйте панель управления для контроля функций сумки
6. Мониторьте данные на главном экране
...
- **Аутентификация:** Протокол идентификации с уникальными ID сумок
